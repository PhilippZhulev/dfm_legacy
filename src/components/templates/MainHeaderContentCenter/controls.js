...
const removeResource = async (event, resourceName, close, request) => {
  try {
    if (event.value === resourceName) {
      request({
        params: {
          resource: event.resource,
          model: event.model,
      ...
      handleWarn({ message: 'Имя узла не совпадает.' });
    }
  } catch (e) {
    handleError('@MainHeaderContentCenter/_removeResource', e);
  }
};
  ...
  try {
    dispatch({
      ...
      text: '',
        ...
        removeResource(e, name, close, request),
    });
  } catch (e) {
